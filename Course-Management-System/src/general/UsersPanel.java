...
public class UsersPanel extends JPanel {
		...
		table.addMouseListener(new MouseAdapter()
		{
			public void mousePressed(MouseEvent e)
			{
				if(e.getClickCount()>1&&e.getButton()==MouseEvent.BUTTON1)
				{
					...
					else if(userprofile.equals("Instructor"))
					{
						int fid=Integer.parseInt(userid);
						Instructor f=new InstructorData().getInstructorInfobyId(fid);
						am.viewinstructorpanel=new ViewInstructorPanel(f,am,am.userspanel);
	...
	public void createtablemodel()
	{
		...
		for(int i=0; i<list.size(); i++)
		{
			...
			model.setValueAt(user.getCurriculumCode()+"-"+user.getSemorYear(), i, 1);
