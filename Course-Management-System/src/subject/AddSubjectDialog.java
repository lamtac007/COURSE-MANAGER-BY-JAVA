...
//Purpose : For adding new subject to curriculum
...
public class AddSubjectDialog extends JDialog implements ActionListener
{
	...
	private JComboBox<String> curriculumtypecombo;
	 ...
	 * Create the dialog.
	 */
	public AddSubjectDialog(String curriculumCode,int sem,JTable datatable) {
		...
		JLabel lblCurriculumType = new JLabel("Curriculum Type");
		lblCurriculumType.setForeground(Color.DARK_GRAY);
		lblCurriculumType.setFont(new Font("Segoe UI", Font.PLAIN, 18));
		lblCurriculumType.setBounds(30, 198, 132, 36);
		contentPanel.add(lblCurriculumType);
		...
		subjectcodefield.setText(new SubjectData().createSubjectcode(Curriculumcode, sem));
		...
		contentPanel.add(practicalmarksfield);
		 ...
		 curriculumtypecombo = new JComboBox<String>();
		curriculumtypecombo.setFont(new Font("Segoe UI", Font.PLAIN, 18));
		curriculumtypecombo.setModel(new DefaultComboBoxModel<String>(new String[] {"---select---", "core", "optional"}));
		curriculumtypecombo.setFocusable(false);
		curriculumtypecombo.setBackground(Color.WHITE);
		curriculumtypecombo.setBounds(172, 199, 330, 40);
		contentPanel.add(curriculumtypecombo);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if(curriculumtypecombo.getSelectedIndex()==0)
		{
			lblError.setVisible(true);
			lblError.setBounds(curriculumtypecombo.getX(), curriculumtypecombo.getY()+curriculumtypecombo.getHeight()-5, 331, 30);
		}
		...
		else if(new SubjectData().isExist(Curriculumcode,semoryear,subjectnamefield.getText()))
		{
			lblError.setVisible(true);
			lblError.setBounds(subjectnamefield.getX(), subjectnamefield.getY()+subjectnamefield.getHeight()-5, 331, 30);
		...
		else
		{
			if(e.getSource()==addsubject)
			{
				...
				String subjecttype=(String) curriculumtypecombo.getSelectedItem();
				...
				su.setCurriculumCode(Curriculumcode);
				...
				if(result==1)
				{
					ResultSet st=new SubjectData().getSubjectinfo(Curriculumcode,semoryear);
