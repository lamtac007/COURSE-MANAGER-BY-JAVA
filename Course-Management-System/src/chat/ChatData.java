...
public class ChatData {
	...
	{
		int total=0;
		String curriculum=s.getCurriculumCode()+" "+new CurriculumData().getsemoryear(s.getCurriculumCode())+"-"+s.getSemorYear();
		try
		{
			String query="select readby from chat where ((touserid='"+curriculum+" Official Group' or touserid='"+curriculum+" Students Group') and fromuserid!='"+s.getUserId()+"') or touserid='"+s.getUserId()+"'";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(query);
			while(rs.next())
			{
	...
	public int getUndreadMessageCountInstructor(Instructor f)
	{
		int total=0;
		String curriculum=f.getCurriculumCode()+" "+new CurriculumData().getsemoryear(f.getCurriculumCode())+"-"+f.getSemorYear();
		try
		{
			String query="select readby from chat where ((touserid='"+curriculum+" Official Group' or touserid='"+curriculum+" Instructors Group') and fromuserid!='"+f.getInstructorId()+"') or touserid='"+f.getInstructorId()+"'";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(query);
			while(rs.next())
			{
				String readbystr=rs.getString(1);
				boolean isreaded=isReadBy(readbystr,f.getInstructorId()+"");
				if(!isreaded)
				{
					total++;
				}
