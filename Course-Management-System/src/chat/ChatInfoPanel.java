...
public class ChatInfoPanel extends JPanel {
	...
	public void setInstructor(Instructor f)
	{
		this.instructor=f;
		headerlabel.setText("Your Info");
	}
	...
	public void setData(Instructor f)
	{
		this.Enable();
		if(instructor!=null && f.getInstructorId()==instructor.getInstructorId())
		{
			onlinestatus.setVisible(true);
		}
		total=0;
		...
		bottomlabel.setText("("+f.getInstructorId()+") "+f.getCurriculumCode()+"-"+f.getSemorYear()+"-"+f.getSubject());
		contactnamelabel.setText(f.getInstructorName());
		headerlabel.setText("Instructor Info");
		if(instructor!=null && f.getInstructorId()==instructor.getInstructorId())
		{
			headerlabel.setText("Your Info");	
		}
	...
	private void membersPanel(Group g)
	{
		...
		{
			JPanel instructorpanel=createPanel(instructor.getProfilePic(),"You","Instructor-"+instructor.getInstructorId(),true);
			memberspanel.add(instructorpanel);
			total++;
		}
	...
	for(ContactInfo c:ContactListPanel.contactinfo)
		{
			...
			if((gn.contains("Students")||gn.contains("Official"))&&c.getClassName().equals("Student"))
			{
				...
				if(s.getCurriculumCode().equals(g.getCurriculumCode())&&s.getSemorYear()==g.getSemorYear())
				{
			...
			else if((gn.contains("Faculties")||gn.contains("Official"))&&c.getClassName().equals("Instructor"))
			{
				Instructor f=c.getInstructor();
				if(f.getCurriculumCode().equals(g.getCurriculumCode())&&f.getSemorYear()==g.getSemorYear())
				{
				JPanel panel=createPanel(f.getProfilePic(),f.getInstructorName(),"Instructor-"+f.getInstructorId(),f.getActiveStatus());
				memberspanel.add(panel);
				total++;
				}
			}
...
//		memberspanel.revalidate();
	...
	public JPanel createPanel(Image image,String username,String lastlogin,boolean isactive)
	{
		...
		else if(lastlogin.contains("Members")||lastlogin.equals("Admin")||lastlogin.contains("Instructor")||lastlogin.contains("Student"))
		{
