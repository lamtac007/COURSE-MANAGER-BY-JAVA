...
public class RollGeneratorPanel extends JPanel {
	...
	public RollGeneratorPanel(String strcurriculumcode,int intsem) {
		setBackground(Color.WHITE);
		this.curriculumcode=strcurriculumcode;
		this.sem=intsem;
		this.setSize(605, 71);
		setLayout(null);
		panel=new JPanel[sem];
		curriculumcodelabel=new JLabel[sem];
		semyearlabel=new JLabel[sem];
		textField=new JTextField[sem];
		
		JLabel lblCurriculumCode = new JLabel("Curriculum Code");
		lblCurriculumCode.setBorder(new LineBorder(new Color(192, 192, 192)));
		lblCurriculumCode.setHorizontalAlignment(SwingConstants.CENTER);
		lblCurriculumCode.setOpaque(true);
		lblCurriculumCode.setForeground(new Color(255, 255, 255));
		lblCurriculumCode.setBackground(new Color(32, 178, 170));
		lblCurriculumCode.setFont(new Font("Segoe UI", Font.BOLD, 20));
		lblCurriculumCode.setBounds(10, 10, 166, 50);
		add(lblCurriculumCode);
		...
		for(int i=0; i<intsem; i++)
		{
				...
				curriculumcodelabel[i] = new JLabel(curriculumcode);
				curriculumcodelabel[i].setOpaque(true);
				curriculumcodelabel[i].setHorizontalAlignment(SwingConstants.CENTER);
				curriculumcodelabel[i].setForeground(Color.DARK_GRAY);
				curriculumcodelabel[i].setFont(new Font("Segoe UI", Font.PLAIN, 20));
				curriculumcodelabel[i].setBorder(new LineBorder(new Color(192, 192, 192)));
				curriculumcodelabel[i].setBackground(new Color(255, 255, 255));
				curriculumcodelabel[i].setBounds(0, 0, 166, 50);
				panel[i].add(curriculumcodelabel[i]);
