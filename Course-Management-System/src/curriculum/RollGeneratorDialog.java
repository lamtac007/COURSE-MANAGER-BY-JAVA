...
public class RollGeneratorDialog extends JDialog implements ActionListener
{
	...
	JComboBox <String>curriculumnamecombo;
	...
	JLabel lblSelectCurriculum,curriculumnamelabel;
	...
	public RollGeneratorDialog() {
		...
		panel.add(lblError);
		 ...
		 curriculumnamecombo = new JComboBox<String>(new CurriculumData().getRollCurriculumName());
		curriculumnamecombo.setForeground(Color.DARK_GRAY);
		...
		curriculumnamecombo.setBounds(182, 93, 407, 37);
		getContentPane().add(curriculumnamecombo);
		
		
		 lblSelectCurriculum = new JLabel("Select Course  :");
		 lblSelectCurriculum.setHorizontalAlignment(SwingConstants.RIGHT);
		lblSelectCurriculum.setForeground(new Color(0, 0, 0));
		lblSelectCurriculum.setFont(new Font("Segoe UI", Font.PLAIN, 18));
		lblSelectCurriculum.setBounds(10, 93, 162, 37);
		getContentPane().add(lblSelectCurriculum);
	...
	public void actionPerformed(ActionEvent e)
	{
		// TODO Auto-generated method stub
			...
			if(e.getSource()==curriculumnamecombo&&curriculumnamecombo.getSelectedIndex()>0)
			{
				...
				{
					sem=new CurriculumData().getTotalsemoryear(""+curriculumnamecombo.getSelectedItem());
					curriculumcode=new CurriculumData().getCurriculumcode(""+curriculumnamecombo.getSelectedItem());
					
					rp=new RollGeneratorPanel(curriculumcode,sem);
					rp.setLocation(0,0);
					rp.setVisible(true);
			...
			else if(e.getSource()==curriculumnamecombo&&curriculumnamecombo.getSelectedIndex()==0)
			{
				if(rollgenscroll!=null)
				{
		...
		if(e.getSource()==btnSave&&!CheckError() && curriculumnamecombo.getSelectedIndex()!=0)
		{
				String curriculumcode=new CurriculumData().getCurriculumcode(""+curriculumnamecombo.getSelectedItem());
				this.adddatatotable(curriculumcode, sem);
				this.dispose();
		}
	...
	public void adddatatotable(String curriculumcode,int limit)
	{
		for(int i=1; i<=limit; i++)
		{
			long roll=Long.parseLong(rp.textField[i-1].getText());
			new RollNumberData().adddata(curriculumcode, i, roll);
